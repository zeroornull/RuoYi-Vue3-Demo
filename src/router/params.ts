...
export function buildRedirectLocation(pathParam: unknown, query: LocationQuery): RouteLocationRaw {
  const parsed = parseSingleRouteParam(pathParam);
  const normalized = parsed?.replace(/^\/+/, "") ?? "";
  const path = normalized.length === 0 || normalized.startsWith("redirect/") ? "/index" : `/${normalized}`;
