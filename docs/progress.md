...
> 下一轮：第 17 轮 — 监控域（可拆 17.a—17.c）
> 状态只能使用：`未开始`、`进行中`、`已完成`、`阻塞`。

## 1. 当前快照

第 **0—16** 轮已经做完。第 **0—11** 轮已入库；第 **12—16** 同在工作区，尚未单独提交。16.d 把角色列表、数据权限、分配用户和选择用户从占位移到真实页面，并接上本地 Mock。

仓库已经不是空壳：根目录可 `dev` / `typecheck` / 三环境 `build`，有 Element Plus、类型化 HTTP/API、7 个 Pinia store、Vue Router 5 权限闭环、响应式 Layout、共享组件、认证业务页，以及系统管理的配置、组织结构、用户和角色授权页。监控与工具尚未迁。
...
| 业务 API | 是。认证 API 已接 user store；16.a—16.d 系统配置/组织/用户/角色已接；监控、工具尚未接页面 |
...
| 登录/CRUD 界面？ | 登录/注册/锁屏/个人中心已迁；16.a—16.d 参数/字典/公告/岗位/部门/菜单/用户/角色已迁 |
| 测试 | `tests/unit`+`tests/system` **161** 条 + integration **11** 条 + contracts **4** 条，共 **176** 条 |
...
打开 [第 17 轮：监控域](./rounds/17-monitoring-pages.md)。下一子批次是 17.a（online / logininfor / operlog）。
...
src/views/                      # 第 15 轮认证页 + 16.a—16.d 系统配置/组织/用户/角色
src/icons/registry.ts           # 显式语义图标映射
src/composables/                # CRUD 选择/分页 + useDict
vite/mock/                      # 开发 Mock：认证 + 16.a—16.d 系统配置/组织/用户/角色
tests/unit/auth/                # 登录、记住密码、RSA、profile/lock 边界
tests/system/                   # 16.a—16.d config/dict/notice/post/dept/menu/user/role 模型与 Mock
...
src/
  ...
  views/                  登录、注册、锁屏、个人中心、401/404、系统 config/dict/notice/post/dept/menu/user/role
...
16.a—16.d 的 config/dict/notice/post/dept/menu/user/role 已接到真实页面。监控/工具 component 仍走白名单占位。首页仍是占位。
...
阶段 E  组件与业务页         第 13—18 轮  做到第 16；17—18 未开始
