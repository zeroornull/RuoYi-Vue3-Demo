...
function notice(method: string, path: string, body?: unknown, query?: Record<string, string>) {
...
describe("notice mock CRUD", () => {
  test("filters by type, creates, updates status and lists read users", () => {
    ...
    expect((typed.body.rows as Array<{ noticeTitle: string }>)[0]?.noticeTitle).toBe("维护通知");
    const top = notice("GET", "/system/notice/listTop");
    expect(top.body.unreadCount).toBe(1);
    expect(notice("POST", "/system/notice", { noticeTitle: "新通知", noticeType: "1" }).body.code).toBe(200);
    expect(notice("POST", "/system/notice", { noticeTitle: "", noticeType: "1" }).body.msg).toBe("公告标题不能为空");
    expect(
    ...
    expect((readers.body.rows as Array<{ userName: string }>)[0]?.userName).toBe("admin");
